...
func _on_area_entered(area: Area2D) -> void:
	if not has_entered:
		...
		hud.hardware_error_instance += 1
		Global.timestamps["hardware_error_enter"][hud.hardware_error_instance] = hud.curr_time
...
func  _on_area_exited(area: Area2D) -> void:
	if has_entered and not has_exited:
		...
		Global.timestamps["hardware_error_exit"][hud.hardware_error_instance] = hud.curr_time
